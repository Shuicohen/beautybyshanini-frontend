...
export const useAdminData = ({ isLoggedIn, token, activeTab, analyticsTimeRange }: UseAdminDataProps) => {
  ...
  // Memoize api functions to prevent infinite loops in useEffect
  const apiInstance = useMemo(() => api, [token, isLoggedIn]);
  ...
  useEffect(() => {
    if (isLoggedIn && token) {
      ...
      setError(null);
      Promise.all([
        apiInstance.get('/api/bookings').catch(err => { if (import.meta.env.DEV) console.error('Bookings error:', err); throw err; }),
        apiInstance.get('/api/services').catch(err => { if (import.meta.env.DEV) console.error('Services error:', err); throw err; }),
        apiInstance.get(`/api/analytics?range=${analyticsTimeRange}`).catch(err => {
          if (import.meta.env.DEV) console.error('Analytics fetch failed:', err);
          return { mostBooked: '', revenueEstimate: 0, vipClients: [] };
        }),
        apiInstance.get('/api/availability/dates').catch(err => { if (import.meta.env.DEV) console.error('Availability error:', err); throw err; }),
      ])
  ...
  }, [isLoggedIn, token, activeTab, apiInstance]);

  // Fetch analytics when time range changes or when Analytics tab becomes active
  useEffect(() => {
    if (isLoggedIn && token && activeTab === 'Analytics') {
      apiInstance.get(`/api/analytics?range=${analyticsTimeRange}`)
  ...
  }, [analyticsTimeRange, activeTab, isLoggedIn, token, apiInstance]);
